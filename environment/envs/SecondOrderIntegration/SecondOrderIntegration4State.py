...
class SecondOrderIntegration4State(rl_base):
    def __init__(self,
        ...
        super(SecondOrderIntegration4State, self).__init__()
        self.name = 'SecondOrderIntegration4State'
        self.init_pos = pos0
        self.init_vel = vel0
        ...
        if self.use_normalization:
            ...
            self.initial_state = np.hstack((self.pos, self.vel))
    ...
    def state_norm(self):
        state = np.hstack((self.pos, self.vel))
    ...
    def get_reward(self, param=None):
        if self.is_controller_BangBang:
            if self.use_normalization:
                cur_s = self.inverse_state_norm(self.current_state)  # pos, vel
            ...
            Qp, Qv, R = 10, 1, 0.1
            r1 = - Qp * np.linalg.norm(nex_s[:2]) ** 2 - Qv * np.linalg.norm(nex_s[2:]) ** 2
            r2 = - R * np.linalg.norm(self.force) ** 2
            r3 = - Qp * np.tanh(10 * np.linalg.norm([nex_s[:2]])) ** 2 - Qv * np.tanh(np.linalg.norm([nex_s[2:]])) ** 2
            # r3 = 0
    ...
    def ode(self, xx: np.ndarray):
        ...
        # 正弦扰动
        # _ddx = self.force[0] - self.k * _dx + 3 * np.sin(5 * self.time)
        # _ddy = self.force[1] - self.k * _dy - 3 * np.sin(5 * self.time)
    ...
    def step_update(self, action: np.ndarray):
        ...
        if self.use_normalization:
            ...
            self.current_state = np.hstack((self.pos, self.vel))
        ...
        if self.use_normalization:
            ...
            self.next_state = np.hstack((self.pos, self.vel))
