...
class CartPole(rl_base):
    def __init__(self, initTheta: float, initX: float, save_cfg: bool=True):
        ...
        self.xMax = 1  # maximum distance
        # self.dxMax = 10                 # maximum velicity

        self.staticGain = 2.0
        self.norm_4_boundless_state = 10
        ...
        self.dt = 0.01  # 10ms
        self.timeMax = 10  # maximum time of each episode
        self.time = 0.
        ...
        '''RL_BASE'''
        self.state_dim = 6  # theta, etheta, dtheta, x, ex, dx
    ...
    def make_text(self):
        ...
        cv.putText(self.image, "theta: %.3f " % (rad2deg(self.theta)), (20, 40), cv.FONT_HERSHEY_COMPLEX, 0.4, Color().Black, 1)
    ...
    def reset_random(self):
        ...
        """
        '''physical parameters'''
        self.initTheta = deg2rad(random.uniform(-self.thetaMax, self.thetaMax))
        self.initX = random.uniform(-self.xMax, self.xMax)
        self.theta = self.initTheta
        self.x = self.initX
        self.dtheta = 0.  # 从左往右转为正
        self.dx = 0.  # 水平向左为正
