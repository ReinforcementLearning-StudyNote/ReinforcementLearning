...
class Nav_EmptyWorld(samplingmap, rl_base):
    def __init__(self, samplingMap_dict: dict, vRange: list, aRange: list, save_cfg: bool):
        ...
        self.a0 = 2
        self.oritination = 0.0  # The direction of the acceleration, 与X轴正方向
        self.oritination_step = 5.0
        self.dt = 0.01
        self.time = 0.0
        ...
        self.state_dim = 2
        self.state_num = [math.inf, math.inf]
        self.state_step = [None, None]
        self.state_space = [None, None]
        self.state_range = [[-self.static_gain, self.static_gain],
                            [-self.static_gain, self.static_gain]]
        self.isStateContinuous = [True, True]

        self.action_dim = 1
        self.action_step = [self.oritination_step]
        self.action_range = [[-180.0, 180.0]]  # 一圈角度
        self.action_num = [int((self.action_range[0][1] - self.action_range[0][0]) / self.action_step[0] + 1)]
        self.action_space = [[self.action_range[0][0] + i * self.action_step[0] for i in range(self.action_num[0])]]
        ...
        self.Ay = [self.a[1]]
        self.Time = [self.time]
        self.Oritination = [self.oritination]
        '''data_save'''
        if save_cfg:
            self.saveModel2XML()
    ...
    def step_update(self, action: list):
        """
        :param action:      action from RL
        :return:            None
        """
        self.oritination = action[0]
        self.current_action = action.copy()
        self.a[0] = self.a0 * cosd(self.oritination)
        self.a[1] = self.a0 * sind(self.oritination)
        self.current_state = [(self.terminalP[0] - self.p[0]) / self.x_size * self.static_gain,
                              (self.terminalP[1] - self.p[1]) / self.y_size * self.static_gain]

        cur_error = self.dis_two_points(self.terminalP, self.p)
        ...
        r3 = 100 if self.terminal_flag == 3 else 0
        '''reward for a successful episode'''
        ...
        '''reward for other'''
        r5 = 0
        # next_error = self.dis_two_points(self.terminalP, self.p)
        # r5 = 100.0 * (cur_error - next_error)
        # if cur_error > next_error:
        #     r5 = 1.0
        # elif cur_error < next_error:
        #     r5 = -1.0
        # else:
        #     r5 = 0
        '''reward for other'''
        self.reward = r1 + r2 + r3 + r4 + r5
        '''reward function'''
        self.saveData()
    ...
    def saveData(self, is2file=False, filename='2D_Nav_EmptyWorld_Continuous.csv', filepath=''):
        ...
        self.Oritination.append(self.oritination)
        if is2file:
            data = pd.DataFrame({
                'Px:': self.Px,
                'Py:': self.Py,
