...
class MARS_UAV_Hover(rl_base, MARS_UAV):
    def __init__(self,
                 pos0=None,
                 vel0=None,
                 angle0=None,
        ...
        self.uav_vis = MARS_UAV_Visualization(xbound=np.array([self.pos_min[0], self.pos_max[0]]),
    ...
    def state_norm(self) -> np.ndarray:
        ...
        norm_angle = (2 * self.angle - self.angle_max - self.angle_min) / (
                    self.angle_max - self.angle_min) * self.staticGain
        norm_dangle = (2 * self.omega_inertial - self.dangle_max - self.dangle_min) / (
                    self.dangle_max - self.dangle_min) * self.staticGain
    ...
    def inverse_state_norm(self, error_flag=False, pos_flag=False, vel_flag=False, angle_flag=False, dangle_flag=False):
        ...
        if pos_flag:
            _s = np.concatenate((_s, (np_cur_state[3:6] / self.staticGain * (
                        self.pos_max - self.pos_min) + self.pos_max + self.pos_min) / 2))
        if vel_flag:
            _s = np.concatenate((_s, (np_cur_state[6:9] / self.staticGain * (
                        self.vel_max - self.vel_min) + self.vel_max + self.vel_min) / 2))
        if angle_flag:
            _s = np.concatenate((_s, (np_cur_state[9:12] / self.staticGain * (
                        self.angle_max - self.angle_min) + self.angle_max + self.angle_min) / 2))
        if dangle_flag:
            _s = np.concatenate((_s, (np_cur_state[12:15] / self.staticGain * (
                        self.dangle_max - self.dangle_min) + self.dangle_max + self.dangle_min) / 2))
        # if f_flag:
        #     _s = np.concatenate((_s, (np_cur_state[15:19] / self.staticGain * (self.fmax - self.fmin) + self.fmax + self.fmin) / 2))

        return _s

    def saveModel2XML(self, filename='MARS_UAV_Hover.xml', filepath='../../environment/config/'):
        rootMsg = {
            'name': 'mars_uav_hover',
            'author': 'Cai Yu',
            'date': '2023.06.18',
            'E-mail': '22S004074@stu.hit.edu.cn'
        }
        rl_baseMsg = {
    ...
    def get_reward(self, param=None):
        ...
        """
        ss = self.inverse_state_norm(error_flag=True, pos_flag=False, vel_flag=False, angle_flag=False,
                                     dangle_flag=False)
        '''
    ...
    def show_dynamic_image(self, per_show=10):
        ...
        @return:            None
        """
        self.uav_vis.render(p=self.pos, ref_p=self.target_pos, v=self.vel, a=self.angle, ra=self.omega_inertial,
                            D=self.D, d=self.d, f=self.thrust, win=per_show)
