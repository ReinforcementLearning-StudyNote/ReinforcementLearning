...
class Two_Wheel_Ground_Vehicle_Continuous(samplingmap, rl_base):
    def __init__(self, initPhi: float, save_cfg: bool, x_size: float, y_size: float, start: list, terminal: list):
        ...
        """
        super(Two_Wheel_Ground_Vehicle_Continuous, self).__init__(width=400,
                                                                  height=400,
                                                                  x_size=x_size,
                                                                  y_size=y_size,
        ...
        self.initial_state = [self.terminal[0] - self.x, self.terminal[1] - self.y, self.x, self.y, self.phi, self.dx, self.dy, self.dphi]
    ...
    def draw_car(self):
        ...
        cv.fillConvexPoly(self.image, points=np.array([list(self.dis2pixel(pt)) for pt in pts_left]), color=Color().DarkGray)
        # right
        ...
        cv.fillConvexPoly(self.image, points=np.array([list(self.dis2pixel(pt)) for pt in pts_right]), color=Color().DarkGray)
        '''两个车轮'''
        # 额外画一个圆形，标志头
        head = [self.r - 0.02, 0]
        head = points_rotate(head, self.phi)
        head = points_move(head, [self.x, self.y])
        cv.circle(self.image, self.dis2pixel(head), self.length2pixel(0.02), Color().Green, -1)  # 主体
