...
class MARS_UAV_Visualization:
    def __init__(self, xbound: np.ndarray, ybound: np.ndarray, zbound: np.ndarray, origin: np.ndarray,
        ...
        self.is_dynamic_axis = max(np.max(np.fabs(xbound)), np.max(np.fabs(ybound)),
                                   np.max(np.fabs(zbound))) >= math.inf
        ...
        self.ax.set_title('MarsHelicopter Fly Simulation', fontsize='13')
        ...
        self.color = ['red', 'orange', 'blue', 'purple']

        self.has_ref = False  # 默认是没有轨迹跟踪的
        self.has_target = True  # 默认有一个目标点

        '''UAV相关部件'''
        self.origin_point = self.ax.plot([], [], [], marker='o', color='black', markersize=6, antialiased=False)[
            0]  # 画原点
        self.target_point = self.ax.plot([], [], [], marker='o', color='red', markersize=10, antialiased=False)[
            0]  # 画终点
        self.center = self.ax.plot([], [], [], marker='o', color='blue', markersize=6, antialiased=False)[0]  # 中心
        self.bar = [self.ax.plot([], [], [], color=self.color[i], linewidth=2.5, antialiased=False)[0] for i in
                    range(4)]
        self.bar_ball = [self.ax.plot([], [], [], marker='o', color=self.color[i], markersize=6, antialiased=False)[0]
                         for i in range(4)]
        ...
        self.text_e = self.ax.text2D(0.02, 0.92 - 0.00, 'Pos_e: {} m'.format([0, 0, 0]), transform=self.ax.transAxes,
                                     fontsize='11')
        self.text_pos = self.ax.text2D(0.02, 0.92 - 0.03, 'Pos: {} m'.format([0, 0, 0]), transform=self.ax.transAxes,
                                       fontsize='11')
        self.text_vel = self.ax.text2D(0.02, 0.92 - 0.06, 'Pos: {} m/s'.format([0, 0, 0]), transform=self.ax.transAxes,
                                       fontsize='11')
        self.text_att = self.ax.text2D(0.02, 0.92 - 0.09, 'Pos: {} deg'.format([0, 0, 0]), transform=self.ax.transAxes,
                                       fontsize='11')
        self.text_att_rate = self.ax.text2D(0.02, 0.92 - 0.12, 'Pos: {} deg/s'.format([0, 0, 0]),
        ...
        self.grav = self.ax.quiver(self.o[0], self.o[1], self.o[2], 0, 0, -1, length=0.8 * 9.8 * self.length_per_n,
                                   color='red')
        self.stick = self.ax.plot([], [], [], color='black', linewidth=4, antialiased=False)[0]  # 连接桨盘和中心的机体
        self.rotor_force = self.ax.quiver(self.o[0], self.o[1], self.o[2], 0, 0, 0, length=1, color='brown')
    ...
    def render(self,
        ...
        if self.sim_index % win == 0:
            ...
            d *= 10
            bar = np.dot(R_b_i, np.atleast_2d([[d0, d0, d], [d0, -d0, d], [-d0, -d0, d], [-d0, d0, d]]).T) + np.vstack(
                (p, p, p, p)).T
            label = np.dot(R_b_i, np.atleast_2d(
                [[d0, d0, 0.3], [d0, -d0, 0.3], [-d0, -d0, 0.3], [-d0, d0, 0.3]]).T) + np.vstack((p, p, p, p)).T
            ...
            '''目标位置 (如果有)'''
            if self.has_target:
                self.quadGui['target_point'].set_data([self.target[0], self.target[0]],
            ...
            '''四个机臂位置'''
            xCenter = R_b_i @ np.array([0, 0, d]) + p  # 机臂X中心位置
            for i in range(4):
                self.quadGui['bar'][i].set_data([bar[:, i][0], xCenter[0]], [bar[:, i][1], xCenter[1]])
                self.quadGui['bar'][i].set_3d_properties([bar[:, i][2], xCenter[2]], [bar[:, i][2], xCenter[2]])
                self.quadGui['bar_ball'][i].set_data([bar[:, i][0], bar[:, i][0]], [bar[:, i][1], bar[:, i][1]])
                self.quadGui['bar_ball'][i].set_3d_properties([bar[:, i][2], bar[:, i][2]],
                                                              [bar[:, i][2], bar[:, i][2]])
            self.quadGui['stick'].set_data([p[0], xCenter[0]], [p[1], xCenter[1]])
            self.quadGui['stick'].set_3d_properties([p[2], xCenter[2]], [p[2], xCenter[2]])
            '''四个机臂位置'''
            ...
            '''升力'''
            self.rotor_force.remove()
            self.rotor_force = self.ax.quiver((bar[:, 0][0] + bar[:, 2][0]) / 2, (bar[:, 0][1] + bar[:, 2][1]) / 2,
                                              (bar[:, 0][2] + bar[:, 2][2]) / 2, dir_f[0], dir_f[1], dir_f[2],
                                              length=f * self.length_per_n, color='brown')
            '''升力'''
        self.sim_index += 1
