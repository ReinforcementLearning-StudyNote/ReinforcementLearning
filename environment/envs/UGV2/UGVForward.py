...
class UGV_Forward(rl_base):
    def __init__(self,
                 ...
                 target: np.ndarray = np.array([4.0, 4.0])):
        super(UGV_Forward, self).__init__()
        ...
        self.static_gain = 2
        self.state_dim = 6  # ex, ey, x, y, dx, dy, phi, dphi
        ...
        if self.use_normalization:
            ...
            self.initial_state = np.append(np.hstack((self.error, self.vel)), [self.phi, self.omega])
        self.current_state = self.initial_state.copy()
        self.next_state = self.initial_state.copy()

        self.action_dim = 2
        self.action_step = [None, None]
        self.action_range = [[self.wMin, self.wMax], [self.wMin, self.wMax]]
    ...
    def state_norm(self):
        state = np.append(np.hstack((self.error, self.vel)), [self.phi, self.omega])
    ...
    def get_reward(self, param=None):
        ...
        if self.sum_d_theta > 6 * np.pi:  # 如果转的超过两圈
        ...
        # k1 = 5
        # r1 = -(yyf_x0 * 4) ** 2 * k1
        if nex_error > cur_error + 1e-3:
            r1 = -3
        elif cur_error > nex_error + 1e-3:
            r1 = 3
        else:
            r1 = 0
        # r1 = -yyf_x0 - np.tanh(2.5 * yyf_x0) + 1
        '''r2 是角度'''
        nex_phi = nex_s[-2]  # 车的朝向角
        cur_theta = cal_vector_rad(cur_s[:2], [np.cos(cur_s[-2]), np.sin(cur_s[-2])])
        nex_theta = cal_vector_rad(nex_s[:2], [np.cos(nex_phi), np.sin(nex_phi)])
        ...
        # else:
        #     r2 = -theta ** 2 * kk2
        # r2 = -nex_theta - np.tanh(2.5 * nex_theta) + 1
        if cur_theta + 1e-2 < nex_theta:
            r2 = -2
        elif cur_theta > nex_theta + 1e-2:
            r2 = 2
        else:
            r2 = 0
        # print(rad2deg(theta))
        # self.reward = - (1 - np.tanh(5 * yyf_x0)) * r1 - np.tanh(5 * yyf_x0) * r2 + r4
        r3 = -1
    ...
    def step_update(self, action: np.ndarray):
        ...
        if self.use_normalization:
            ...
            self.current_state = np.append(np.hstack((self.error, self.vel)), [self.phi, self.omega])

        '''rk44'''
        aRef = np.clip((action - self.w_wheel) / self.dt, self.aMin, self.aMax)
        ...
        if self.use_normalization:
            ...
            self.next_state = np.append(np.hstack((self.error, self.vel)), [self.phi, self.omega])
        self.get_reward()
    ...
    def reset(self):
        ...
        self.current_state = self.initial_state.copy()
        self.next_state = self.initial_state.copy()
        self.initial_action = self.w_wheel.copy()
        self.current_action = self.initial_action.copy()
        self.reward = 0.0
        self.is_terminal = False
        self.terminal_flag = 0  # 0-正常 1-出界 2-超时 3-成功
