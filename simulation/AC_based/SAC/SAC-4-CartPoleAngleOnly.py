...
class DualCritic(nn.Module):
    def __init__(self, beta, state_dim, action_dim, name, chkpt_dir):
        ...
        self.L11 = nn.Linear(self.state_dim + self.action_dim, 64)
        # self.batch_norm11 = nn.LayerNorm(64)
        self.L12 = nn.Linear(64, 64)
        # self.batch_norm12 = nn.LayerNorm(64)
        self.L13 = nn.Linear(64, 1)

        self.L21 = nn.Linear(self.state_dim + self.action_dim, 64)
        # self.batch_norm21 = nn.LayerNorm(64)
        self.L22 = nn.Linear(64, 64)
        # self.batch_norm22 = nn.LayerNorm(64)
        self.L23 = nn.Linear(64, 1)
...
class ProbActor(nn.Module):
    def __init__(self, alpha, state_dim, action_dim, name, chkpt_dir):
        ...
        self.l1 = nn.Linear(self.state_dim, 64)  # 输入 -> 第一个隐藏层
        # self.batch_norm1 = nn.LayerNorm(64)

        self.l2 = nn.Linear(64, 64)  # 第一个隐藏层 -> 第二个隐藏层
...
if __name__ == '__main__':
    ...
    if TRAIN:
        ...
        for p in agent.target_critic.parameters():   # target 网络不训练
        ...
        while agent.episode <= MAX_EPISODE:
            ...
            if timestep % 500 == 0:
                ...
                os.mkdir(temp)
                time.sleep(0.01)
                agent.actor.save_checkpoint(name='Actor_sac', path=temp, num=timestep)
                agent.critic.save_checkpoint(name='Critic_td3', path=temp, num=timestep)
                agent.target_critic.save_checkpoint(name='TargetCritic_td3', path=temp, num=timestep)
            if c == 27:
                print('Over......')
                break
        '''dataSave'''
