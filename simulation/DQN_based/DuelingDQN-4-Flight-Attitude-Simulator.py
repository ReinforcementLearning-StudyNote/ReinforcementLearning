    ...
    while dueling_dqn.replay_count < fullFillCount:
        ...
        while not env.is_terminal:
            ...
            if is_only_success:
                ...
                dueling_dqn.memory.store_transition(env.current_state, env.current_action, env.reward, env.next_state, 1 if env.is_terminal else 0)
                if dueling_dqn.replay_count % 100 == 0:
                    print('replay_count = ', dueling_dqn.replay_count)
        if is_only_success and env.terminal_flag == 3:
            dueling_dqn.memory.store_transition_per_episode(_new_state, _new_action, _new_reward, _new_state_, _new_done)
            print('replay_count = ', dueling_dqn.replay_count)
...
if __name__ == '__main__':
        ...
        while dueling_dqn.episode <= MAX_EPISODE:
            ...
            while not env.is_terminal:
                ...
                env.current_state, env.current_action, env.reward, env.next_state, env.is_terminal = \
                    env.step_update(dueling_dqn.actionNUm2PhysicalAction(numAction))  # 环境更新的action需要是物理的action
            ...
            if is_storage_only_success and env.terminal_flag == 3:
                ...
                dueling_dqn.memory.store_transition_per_episode(new_state, new_action, new_reward, new_state_, new_done)
            '''跳出循环代表回合结束'''
            print(
                '=========START=========',
    ...
    if TEST:
        print('TESTing...')
        dueling_dqn.get_optimalfrompkl(optPath + 'dueling_dqn-4-flight-attitude-simulator.pkl')
        ...
        for i in range(simulation_num):
            ...
            while not env.is_terminal:
                ...
                env.current_state = env.next_state.copy()
                env.current_state, env.current_action, env.reward, env.next_state, env.is_terminal = \
                    env.step_update(dueling_dqn.actionNUm2PhysicalAction(dueling_dqn.get_action_with_fixed_epsilon(env.current_state, 0.0)))
                env.show_dynamic_image(isWait=False)
                cap.write(env.save)
